...
Поддерживаются флаги `APPIUM_NO_RESET` и `APPIUM_NEW_COMMAND_TIMEOUT` в `.env.<context>`.
...
Дополнительно можно задать `BS_PROJECT_NAME` и `BS_BUILD_NAME` через `.env.bstack`.
